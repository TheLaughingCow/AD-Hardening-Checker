    ...
    elseif ($RemediationList -contains "FAILED") {
        if (Test-Path $Settings.CsvPath) {
            try {
                ...
                $failedResults = $auditResults | Where-Object { $_.Status -eq "FAIL" }
                $failedChecks = @()
                ...
                Write-Host "FAILED option detected: $($failedChecks.Count) remediations for failures" -ForegroundColor Yellow
            }
            catch {
