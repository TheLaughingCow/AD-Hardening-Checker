...
elseif ($Mode -eq "Remediation") {
    ...
    if ($RemediationList -contains "ALL") {
        # Mapping des IDs vers les noms de fonctions pour le mode ALL
        $idToFunctionMap = @{
            1 = "LLMNR"
            2 = "NBTNS"
            3 = "mDNSBonjour"
            4 = "MachineAccountQuota"
            5 = "SMBSigning"
            6 = "LDAPSigning"
            7 = "PrintSpooler"
            8 = "SMBv1"
            9 = "LAPS"
            10 = "UnconstrainedDelegation"
            11 = "ProtectedUsersGroup"
            12 = "LSASSProtectedMode"
            13 = "SMBNullSession"
            14 = "LDAPAnonymousBind"
            15 = "PasswordPolicy"
            16 = "RIDBruteForceMitigation"
            17 = "PreWin2000CompatibleAccess"
            18 = "IPv6Management"
            19 = "NTLMRestriction"
            20 = "ShareACLRestriction"
            21 = "DefaultCredentials"
            22 = "KerberosPreAuth"
            23 = "CoercionPatches"
            24 = "TieringModel"
            25 = "PasswdNotReqdFlag"
            26 = "SecureServiceAccounts"
            27 = "SecurityBaseline"
        }
        
        $RemediationList = @()
        foreach ($id in $Settings.AllowedRemediations) {
            if ($idToFunctionMap.ContainsKey($id)) {
                $RemediationList += $idToFunctionMap[$id]
            }
        }
    ...
    elseif ($RemediationList -contains "PRIORITY") {
        # Mapping des IDs vers les noms de fonctions pour le mode PRIORITY
        $idToFunctionMap = @{
            1 = "LLMNR"
            2 = "NBTNS"
            3 = "mDNSBonjour"
        ...
        $RemediationList = @()
        foreach ($id in $Settings.QuickWinPriority) {
            if ($idToFunctionMap.ContainsKey($id)) {
                $RemediationList += $idToFunctionMap[$id]
            }
        }
        ...
        if (Test-Path $Settings.CsvPath) {
            try {
                ...
                foreach ($result in $failedResults) {
                    ...
                    $action = $action.Trim()  # Remove leading/trailing whitespace
                    ...
                    if ($actionMap.ContainsKey($action)) {
                        ...
                        if ($fallbackMap.ContainsKey($checkName)) {
                            $checkName = $fallbackMap[$checkName]
                        }
                    }
