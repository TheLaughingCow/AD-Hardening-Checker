function Check-MachineAccountQuota {
    ...
    try {
        ...
        if ($quota -eq 0) {
            ...
            $result.Status = "FAIL"
            $result.Action = "MachineAccountQuota Not Secured"
            $result.DetectedValue = "MachineAccountQuota = $quota (allows up to $quota computer accounts per user - should be 0 for security)"
        }
    }
    catch {
        $result.Status = "WARN"
