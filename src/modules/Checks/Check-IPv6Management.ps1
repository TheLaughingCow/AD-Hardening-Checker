    ...
    try {
        ...
        elseif ($ipv6Enabled -and $ipv6ServiceRunning) {
            if ($isDomainController) {
                $result.Status = "WARN"
                $result.Action = "IPv6 Active on DC"
                $result.DetectedValue = "IPv6 active on Domain Controller (recommended to keep IPv6 enabled for AD/DNS compatibility)"
            } else {
                $result.Status = "FAIL"
                $result.Action = "IPv6 Active (Unhardened)"
                $activeList = ($result.ActiveIPv6 | ForEach-Object { $_.IPAddress }) -join ", "
                $result.DetectedValue = "IPv6 active: $activeList (consider disabling if not required or applying RA/DHCPv6 Guard)"
            }
        }
        else {
            $result.Status = "WARN"
            $result.Action = "IPv6 Not Detected"
            $result.DetectedValue = "No IPv6 addresses detected on active interfaces"
        }
    }
    catch {
        $result.Status = "WARN"
